...
# ╔═╡ 572ba5c0-a367-11ec-3a90-37ca3b98ddcb
using Plots, Random, Roots, FFTW, LaTeXStrings

# ╔═╡ 23e23acc-02aa-4378-8947-eacdba541344
cd(dirname(@__FILE__));

# ╔═╡ 404bf700-eb7d-49a7-be3c-9465c6a6e5be
## Parameters

# ╔═╡ 437a909e-d393-452e-ae7c-8fb7f405873d
gam0 = 1;

# ╔═╡ 132fb823-c5e6-40b9-9d6b-b613a17da446
gamg0 = 0.01; # inverse is 1 ns physical

# ╔═╡ 585287e7-c0b9-47a9-aed5-e8c31df5ebac
tau0 = 2; # T

# ╔═╡ d5dbfb4c-55d5-446f-bd54-f65ccd32b10d
alpha0 = 0; # task 1.1/2/3 linewidth enhancement 

# ╔═╡ 32ded54f-48a1-40f6-9753-797cc8813675
kap0 = 0.27; # kappa

# ╔═╡ 321f8169-39aa-4bb5-a95a-0e5bdf1ab459
phi0 = 0; # task 1.1/2 optical phase (!!!)

# ╔═╡ 49d5812f-f863-44bb-8edd-8de55147fb78
g00rel = -5*log(kap0); # task1.1/2

# ╔═╡ 1d608134-87d4-4fad-9962-429169533c5d
g00 = g00rel * 1.0;

# ╔═╡ 7e6491c4-cecf-4680-90bd-937fafb90431
tphys = 1 * 10^(-9) * gamg0;

# ╔═╡ ef1420bc-cbf2-402e-b451-ad73cb16d928
# Initial parameter vector

# ╔═╡ 07f1c399-2aa7-4ce0-a329-77f76817196c
p0 = [gam0, gamg0, tau0, alpha0, kap0, phi0, g00];

# ╔═╡ e3987e8a-874b-4e20-bd9c-450fd6de92cc
t0 = 0;

# ╔═╡ 7afba5ea-acc9-47ad-b0ac-68bc2af80a93
# Integration procedure parameters

# ╔═╡ 600e4759-fa86-4c56-b170-90b2411ea996
dt = tau0 / 1000;

# ╔═╡ 4b34578d-1ca3-4702-8b3d-47fac55b5237
hist_len = floor(Int, tau0 / dt);

# ╔═╡ 1bb141a2-5853-4b67-998b-3dec44ec0b2f
endtime = 2000 * tau0;

# ╔═╡ fb13f849-abb8-4ea7-8f94-693d4d52656d
num_iter = floor(Int, endtime / dt);

# ╔═╡ 8983cdd3-90ca-4e03-8016-5b045a6a7b49
dim = 3; # problem dimension

# ╔═╡ 5597509b-34c2-4f8d-96ae-bfce593d20f2
savnum = 100; # number of points per delay to save

# ╔═╡ 965af078-1f67-407a-a2c6-0ddcb9f470c9
modstep = floor(Int, hist_len / savnum);

# ╔═╡ b282b66d-820e-4a97-81fd-29f1be3f3b60
# Noise parameters

# ╔═╡ 664fa099-2574-42ea-82ed-04ea12b126bb
beta1 = 0.5 * 10^-3 / sqrt(dt); # noise in u[1]

# ╔═╡ ac5fa270-989b-4658-a3a1-bc795a7c1dd8
beta2 = 0.5 * 10^-3 / sqrt(dt); # noise in u[2]

# ╔═╡ 4415f82b-4bc4-4d1a-8bf1-3dd81a2be8ae
...
# ╔═╡ c232b1b7-343e-4407-b30f-c1dd4d5125ee
...
# ╔═╡ 11ed3f41-73b3-49d5-b2b2-951217f7a3cb
...
# ╔═╡ 2dc9f72d-95fc-4b8f-aacc-18c43f00a78f
...
# ╔═╡ f451b6fb-8e22-4039-9a66-730972509cec
## find a steady state as initial condition

# ╔═╡ 0453e51d-47dd-46f4-af49-847e9dd0f8fa
# mode number
M = 0;

# ╔═╡ 585fa888-32d3-43c8-b03f-cdf830b723d4
...
# ╔═╡ cb9ded28-a5ec-4462-b6b3-99ee480e3dae
gsol(ws) = log((ws^2 + p0[1]^2) / (p0[1]^2 * p0[5]));

# ╔═╡ 8aab666b-00d8-4b63-b5db-7fc7952e903c
asol(gs) = sqrt(p0[7] - gs) / sqrt(-1 + exp(gs));

# ╔═╡ 02a78f5d-e743-4fef-b11c-053fe6fc1a1b
om = find_zero(fsol, 2 * pi * M / p0[3]);

# ╔═╡ a7c15cb5-0db1-4722-b212-1fae89ff7e8c
gg = gsol(om);

# ╔═╡ 4531a3ee-4122-4e15-9241-0d11dd3e80b2
p0[7]

# ╔═╡ dc9963fc-d8ed-40e0-bb17-947c92597379
gg

# ╔═╡ ea23a436-628d-4703-a490-6ba8d78215e2
amp = asol(gg);

# ╔═╡ 41d1d2f2-3913-4fe7-8dc9-2a286251a155
amp

# ╔═╡ 32a5e852-c3d8-4a71-9b08-0417033cfc12
# our initial history for SIE method subiterations
qY1 = zeros(dim, hist_len);

# ╔═╡ c94cf367-8b8e-4706-940c-e2c738767c0d
qY2 = zeros(dim, hist_len);

# ╔═╡ 8577116e-52c7-4934-a65a-ac1136347802
for i = 1:hist_len
    qY1[:, i] = [amp * cos(om * dt * (-i)), amp * sin(om * dt * (-i)), gg]
    qY2[:, i] = [amp * cos(om * dt * (1 - i)), amp * sin(om * dt * (1 - i)), gg]
end

# ╔═╡ 76958716-62b9-4f9e-87ab-36d1bec70318
display(plot(
    qY1[1, :] .^ 2 + qY1[2, :] .^ 2,
    ylims = (0, 1.05 * maximum(qY1[1, :] .^ 2 + qY1[2, :] .^ 2)),
))

# ╔═╡ 39288da5-5b00-4485-8d64-7be31e0f0c3e
## Integration

# ╔═╡ 5897d35e-3606-4cbe-91cc-88c5a4873754
U = zeros(div(num_iter, modstep), dim)

# ╔═╡ dd748b78-ab51-4699-8c4b-067b57117e5f
qYtail = [1];

# ╔═╡ bbb49718-0092-4109-be23-9215d2a3eb7c
Random.seed!(1234)

# ╔═╡ a6b6f495-f0a3-46aa-8c08-da589db1ae68
time_array = [t0, t0, dt]

# ╔═╡ 9e475c48-3b36-4979-b333-dbb701f3c4d5
# noise terms

# ╔═╡ 256ce3e9-db14-4526-8042-527567d3b071
nt = zeros(num_iter, 2)
...
# ╔═╡ 48360aca-adf2-4358-bef9-2dd8d99ca52d
# Optical spectrum

# ╔═╡ f96619f4-7555-461e-ad3a-2b8fb397a181
optfreq =
    fftshift(fftfreq(
...
# ╔═╡ cec41ac5-ba72-4c6b-b974-859e1d0b2f66
optspec =
    10 *
...
# ╔═╡ 5fba6b71-febf-4492-b1e6-6310c39be73f
# RF Spectrum

# ╔═╡ 307f583a-5bd0-4793-a133-2c2da52dc77b
rffreq =
    rfftfreq(length(U[pl_st_in:pl_en_in, 1]), 1 / (dt * modstep * tphys)) ./
    10^9;

# ╔═╡ a47a7d23-2432-4340-94cd-2cd4a92cca25
rfspec =
    10 *
    log10.(abs2.(rfft(
...
# ╔═╡ d5a8110e-628f-42bf-9060-2da564a0979e
p2 = plot(
    optfreq,
    optspec,
...
# ╔═╡ b65749a5-46f4-4df6-bd09-3d403030540a
p3 = plot(
    rffreq,
    rfspec,
...
# ╔═╡ a7cae0b4-5f62-42c6-a5a4-584e95a69b44
pall = plot(p1, p2, p3, layout = (3, 1), size = (400, 600), dpi = 300)

# ╔═╡ ff18f47e-90f7-404f-9518-15946e17b8b7
savefig(pall, "graphs/results1.3.1.png")
